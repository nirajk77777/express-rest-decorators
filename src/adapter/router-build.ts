/**
 * Router-build module: path composition, v4 footgun detection,
 * per-controller express.Router() construction, and async per-route
...
/**
 * HandlerFactory type. The third param is the fully-resolved interceptor chain
 * for this route ([...globalInterceptors, ...controllerInterceptors, ...methodInterceptors]).
 */
...
/**
 * Router options — structured options object for additive extension.
 */
...
/**
/**
 * Build one express.Router() per controller. Validates every composed route path
 * with detectV4Pattern() BEFORE registering with the router (ensures users see
 * our v8-suggestion error, not p2re's terse one).
 *
 * Async. Builds per-route handler arrays:
 *   [...ctrlBefore, ...methodBefore, authGate?, invokeHandler, ...methodAfter, ...ctrlAfter]
 *
 ...
 * @param options Structured router options.
 */
export async function buildControllerRouter(
  ...
  for (const action of controllerMeta.actions) {
    ...
    // Method-wins: if action.authorized is explicitly set (including null),
    ...
    // Multer middleware mounts BEFORE invokeHandler so req.files is populated
